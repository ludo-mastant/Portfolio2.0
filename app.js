/* ===========================
   ...
   const PROJECTS = [
     {
       ...
       screens: [
         "./assets/woodycraft/01-home.png",
         "./assets/woodycraft/02-product.png",
         "./assets/woodycraft/03-cart.png",
         "./assets/woodycraft/04-categories.png",
       ],
     ...
     },
     {
       ...
       links: [{ label: "Repo", href: "https://github.com/EvanSurGit/Parcours-de-certification" }],
     },
     {
       id: "p4",
       ...
       galleryFit: "contain",
       screens: [
        "./assets/stage-cti/Bastion.png",
        "./assets/stage-cti/GTI.png",
        "./assets/stage-cti/GDE_HP_Apperçu.png",
        "./assets/stage-cti/PMF_Bulle.png",
        "./assets/stage-cti/QualNet.png",
        "./assets/stage-cti/RMEP_Prod.png",
        "./assets/stage-cti/IMG_CR_auto.png",
        "./assets/stage-cti/IMG_morc_script_CR.png",
        "./assets/stage-cti/GDC_Accueil.png",
        "./assets/stage-cti/GDC_Import_Diffusion.png",
        "./assets/stage-cti/GDC_Import_Diffusion2.png",
        "./assets/stage-cti/Mail_Diffusion_dmd.png",
        "./assets/stage-cti/Mail_Diffusion_Lot.png",

      ],
       meta: ["Missions : MEP", "Scripts Unix/Linux", "Réunions versions/incidents"],
       proof: [
        "Captures d’outils (supervision, ticketing, terminal SSH) — anonymisées",
        "Journal de bord",
   ...
   /* ✅ CERTIFICATIONS : modifie juste ici */
   const CERTIFS = [
     {
       title: "SecNumAcadémie (exemple)",
       org: "ANSSI",
       date: "2025",
   ...
   function openModal(projectId) {
     ...
     galleryIndex = 0; // reset image index when opening
   ...
   function closeModal() {
     ...
     // ✅ clean keyboard handler for gallery
   ...
   function fillModal(p, currentList) {
     ...
     gallery.innerHTML = "";
     gallery.classList.toggle("gallery--contain", p.galleryFit === "contain");

   
     const screens = Array.isArray(p.screens) ? p.screens : [];
     if (screens.length) {
       gallery.classList.add("is-on");
       galleryIndex = 0; // reset when switching project
       ...
       img.loading = "lazy";
       img.src = screens[galleryIndex];
   
       // click = open full image
       img.style.cursor = "zoom-in";
       img.addEventListener("click", () => window.open(screens[galleryIndex], "_blank", "noreferrer"));
   
       const nav = document.createElement("div");
       nav.className = "gallery__nav";
       nav.innerHTML = `
         <button class="gallery__btn" type="button" data-gprev aria-label="Image précédente">←</button>
         <button class="gallery__btn" type="button" data-gnext aria-label="Image suivante">→</button>
       `;
   
       const meta = document.createElement("div");
       meta.className = "gallery__meta";
       meta.textContent = `${galleryIndex + 1}/${screens.length}`;
       ...
       thumbWrap.className = "gallery__thumbWrap";
       ...
       btnL.type = "button";
       btnL.className = "gallery__thumbScroll gallery__thumbScroll--left";
       btnL.textContent = "←";
       btnL.addEventListener("click", () => thumbs.scrollBy({ left: -260, behavior: "smooth" }));
       
       const btnR = document.createElement("button");
       btnR.type = "button";
       btnR.className = "gallery__thumbScroll gallery__thumbScroll--right";
       btnR.textContent = "→";
       thumbWrap.appendChild(btnR);
       btnR.addEventListener("click", () => thumbs.scrollBy({ left: 260, behavior: "smooth" }));
       
       thumbWrap.appendChild(btnL);
       thumbWrap.appendChild(thumbs);
       thumbWrap.appendChild(btnR);
       
       // ... (ton screens.forEach qui append les thumbnails dans `thumbs` reste pareil)
       
       gallery.appendChild(main);
       gallery.appendChild(thumbWrap);
          
       const renderGallery = (i) => {
         galleryIndex = (i + screens.length) % screens.length;
         img.src = screens[galleryIndex];
         meta.textContent = `${galleryIndex + 1}/${screens.length}`;
         $$(".gallery__thumb", thumbs).forEach((t, idx) => t.classList.toggle("is-active", idx === galleryIndex));
       };
   
       screens.forEach((src, idx) => {
         const t = document.createElement("button");
         t.type = "button";
         t.className = "gallery__thumb" + (idx === galleryIndex ? " is-active" : "");
         t.innerHTML = `<img src="${src}" alt="Miniature ${idx + 1}" loading="lazy" />`;
         t.addEventListener("click", () => renderGallery(idx));
         thumbs.appendChild(t);
       });
   
       nav.querySelector("[data-gprev]").addEventListener("click", () => renderGallery(galleryIndex - 1));
       nav.querySelector("[data-gnext]").addEventListener("click", () => renderGallery(galleryIndex + 1));
   
       gallery.appendChild(main);
       gallery.appendChild(thumbs);
   
       // ✅ keyboard arrows for gallery (clean previous)
       ...
       gallery.classList.remove("is-on");
     }
   ...
   /* ---------------------------
      Navigation helper (CmdK + buttons)
   ---------------------------- */
   function goTo(id) {
   ...
   /* Quick buttons */
   $("#btnFocusProjects")?.addEventListener("click", () => goTo("#projets"));
   $("#btnFocusSkills")?.addEventListener("click", () => goTo("#competences"));
   $("#btnFocusContact")?.addEventListener("click", () => goTo("#contact"));
   ...
   $("#contactForm")?.addEventListener("submit", (e) => {
     ...
     const mailto = `mailto:ludo-mastantuono@outlook.fr?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
   ...
   const COMMANDS = [
     { title: "Aller aux Projets", desc: "Voir les réalisations", tag: "section", action: () => goTo("#projets") },
     { title: "Aller aux Compétences", desc: "Mapping & preuves", tag: "section", action: () => goTo("#competences") },
     { title: "Aller à la Veille", desc: "Sujets suivis", tag: "section", action: () => goTo("#veille") },
     { title: "Aller au Parcours", desc: "Timeline", tag: "section", action: () => goTo("#parcours") },
     { title: "Aller aux Certifications", desc: "Preuves en PDF", tag: "section", action: () => goTo("#certifications") },
     { title: "Aller au Contact", desc: "Écrire un message", tag: "section", action: () => goTo("#contact") },
